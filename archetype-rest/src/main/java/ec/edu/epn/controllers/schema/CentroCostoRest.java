...
public class CentroCostoRest {
     ...
     * @return guardar
     */
    @PostMapping(value = "guardarCentroCosto")
    public ResponseEntity<?> create(@RequestBody CentroCostoDto obj) {
        CentroCostoDto dto = service.create(obj);
        ResponseGenerico<CentroCostoDto> response = new ResponseGenerico<>();
        try {
     ...
     */
    @GetMapping(value = "listarCentroCosto")
    public ResponseEntity<?> listar() {
        ResponseGenerico<CentroCostoDto> response = new ResponseGenerico<>();
        try {
            List<CentroCostoDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    @GetMapping(value = "buscarPorIdCentroCosto/{id}")
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<CentroCostoDto> response = new ResponseGenerico<>();
        try {
            CentroCostoDto dto = service.findById(id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
    ...
    @DeleteMapping(value = "eliminarCentroCosto/{id}")
